...
- **Dynamic quorum from live supply (mitigated).** Quorum is now snapshotted at scorecard submission time (`submitScorecardFor`) and stored in `scorecard.quorumSnapshot`. `stateOf` uses this snapshot instead of calling `quorum()` live. Reserve mints or burns after submission cannot change the threshold for that scorecard. A tier counts toward quorum if it has circulating supply (`currentSupplyOfTier > 0`) OR pending reserves (`numberOfPendingReservesFor > 0`). The pending reserves check handles the edge case where all paid tokens in a tier were burned during REFUND — `currentSupplyOfTier` drops to 0, but pending reserves persist because they're tracked separately from circulating supply. The reserve beneficiary still has a stake in the tier's outcome.
