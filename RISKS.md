...
- **Dynamic quorum from live supply (mitigated).** Quorum is now snapshotted at scorecard submission time (`submitScorecardFor`) and stored in `scorecard.quorumSnapshot`. `stateOf` uses this snapshot instead of calling `quorum()` live. Reserve mints or burns after submission cannot change the threshold for that scorecard. The `quorum()` function still computes from live supply for informational/UI purposes, but it is no longer used in the ratification path.
