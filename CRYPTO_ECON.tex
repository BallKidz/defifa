...
\subsection{Governance Deadlock and Fund Recovery}

\textbf{Severity: Critical.}

The protocol defines enum values \texttt{NO\_CONTEST} and \texttt{NO\_CONTEST\_INEVITABLE}, and the cash-out hook correctly handles them (returning full mint-price refunds). However, the \texttt{currentGamePhaseOf} function in \texttt{DefifaDeployer} contains \emph{no code path} that returns either phase. If no scorecard is ever ratified---because participation is too low for quorum, holders cannot reach consensus, or the event outcome is genuinely ambiguous---the game remains in SCORING permanently.

In SCORING without a ratified scorecard, all tier weights are zero, meaning \texttt{cashOutWeightOf} returns~0 for every token. Players receive nothing upon cash-out and cannot access the refund mechanism (which requires MINT, REFUND, or NO\_CONTEST phase). \textbf{Funds are permanently and irrecoverably locked.}

This is the most structurally important finding. Every prediction game has a nonzero probability of governance deadlock. Without a backstop, the expected loss scales linearly with game volume.

\textbf{Recommended fix.} Add a time-bounded fallback parameter $\tau_{\text{no\_contest}}$ (e.g., 30--90 days). If no scorecard is ratified within this window after scoring begins, the game enters NO\_CONTEST automatically:
\begin{equation}
\text{phase} = \begin{cases} \text{NO\_CONTEST} & \text{if } t > t_{\text{start}} + \tau_{\text{no\_contest}} \;\wedge\; \neg\texttt{cashOutWeightIsSet} \\ \text{SCORING} & \text{otherwise} \end{cases}
\end{equation}
...
\textbf{Governance Security.} The attestation model achieves a balance between decentralization and efficiency. However, Section~9 identifies two critical governance vulnerabilities: the lack of a NO\_CONTEST fallback (9.1) means governance deadlock permanently locks funds, and cheap cross-tier attestation capture (9.2) allows an attacker buying 1 token in each of $N/2$ unpopular tiers to unilaterally meet quorum. These findings demonstrate that governance security depends critically on participation uniformity across tiers, not just tier count and prices.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Deadlock protection}: Until a NO\_CONTEST timeout is implemented (Section~9.1), game organizers should establish clear off-chain deadlock-resolution procedures.
\end{enumerate}

\textbf{Synthesis.} Defifa implements a rigorous approach to prediction gaming through the composition of three well-understood mechanisms: parimutuel pooling for price formation, attestation governance for outcome resolution, and Juicebox V5 for treasury management. The mathematical analysis confirms that the system conserves value and converges to informationally efficient equilibria. The open problems identified in Section~9---particularly the governance deadlock risk and cheap cross-tier capture---represent the most important areas for protocol hardening before production deployment at scale. The recommended mitigations (NO\_CONTEST timeout, minimum-supply quorum thresholds, exact weight validation) are backwards-compatible and address the identified vulnerabilities without altering the core mechanism design. The elegance of Defifa resides in its architectural composability: prediction games with arbitrary outcomes, arbitrary tier structures, and arbitrary payout distributions emerge from the same set of parameters, executed deterministically by immutable smart contracts with a single, time-bounded governance input. Addressing the open problems will strengthen this foundation for high-stakes deployment.
