...
\textbf{Historical context.} The original Defifa (Juicebox~V3 era) included \texttt{NO\_CONTEST} and \texttt{NO\_CONTEST\_INEVITABLE} phases. In~V3, each game phase had to be manually advanced by calling \texttt{queueNextPhaseOf()}. If nobody called this function before a funding cycle ``rolled over'' (repeated instead of advancing), the \texttt{\_noContestInevitable()} check detected the rollover and \texttt{\_queueNoContest()} reconfigured the project for permanent full-price refunds. The~V5 port pre-queues all rulesets at launch, eliminating the rollover risk---but also eliminating the sole trigger for no-contest. The dead enum values and handler code were removed as part of the~V5 cleanup.

\textbf{Current state.} If no scorecard is ever ratified---because participation is too low for quorum, holders cannot reach consensus, or the event outcome is genuinely ambiguous---the game remains in SCORING indefinitely. In SCORING without a ratified scorecard, all tier weights are zero, meaning cash-outs yield nothing. The game's funds remain in the Juicebox treasury with no automated recovery path.

\textbf{Mitigating factors.} Unlike an exploit, governance deadlock is publicly observable. Game organizers and the \texttt{defaultAttestationDelegate} have strong social incentives to ratify a reasonable scorecard (even a full-refund proportional scorecard). The scoring phase has no time limit, giving participants unlimited time to coordinate.

\textbf{Potential improvement.} A time-bounded fallback could be added: if no scorecard is ratified within a configurable window after scoring begins (e.g., 90~days), cash-outs could default to returning mint prices.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Deadlock awareness}: Game organizers should establish clear scorecard-resolution procedures for disputed or low-participation games (Section~9.1).
\end{enumerate}
