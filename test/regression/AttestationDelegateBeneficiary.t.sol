...
contract AttestationDelegateBeneficiary is JBTest, TestBaseWorkflow {
    ...
    function test_defaultDelegateIsBeneficiaryNotPayer() public {
        ...
        // H-6 fix: delegation for the payer should point to the beneficiary, not to the payer.
        address delegate = _nft.getTierDelegateOf(payer, 1);
        assertEq(delegate, beneficiary, "H-6: default delegate should be beneficiary, not payer");
        assertTrue(delegate != payer, "H-6: delegate must not be payer when payer != beneficiary");
    }
    ...
    function test_explicitDelegateOverridesDefault() public {
        ...
        address delegate = _nft.getTierDelegateOf(payer, 1);
        assertEq(delegate, explicitDelegate, "Explicit delegate should override default");
    }
