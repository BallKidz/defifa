...
/// @notice Verifies the fix for H-1: Same-block transfer no longer enables double attestation.
/// After the fix, attestToScorecardFrom uses block.timestamp - 1 as the snapshot, so transfers
/// within the current block are invisible to the attestation weight lookup.
contract FixSameBlockDoubleAttestTest is JBTest, TestBaseWorkflow {
    ...
    /// @notice After the fix, a same-block transfer recipient gets zero attestation weight because
    /// the snapshot at block.timestamp - 1 does not include the current block's transfer.
    function test_sameBlockTransferRecipientGetsZeroAttestationWeight() external {
        ...
        assertGt(originalWeight, 0, "original holder should have attestation weight");

        // Transfer in the same block.
        vm.prank(originalHolder);
        _nft.transferFrom(originalHolder, recipient, 1_000_000_001);

        // Recipient tries to attest in the same block. With the fix (snapshot at block.timestamp - 1),
        // the recipient's weight should be zero because the transfer happened in the current block.
        vm.prank(recipient);
        uint256 recipientWeight = _gov.attestToScorecardFrom(_gameId, proposalId);
        assertEq(recipientWeight, 0, "recipient should get zero weight for same-block transfer");

        // Total attestation should only reflect the original holder's weight.
        assertEq(
            _gov.attestationCountOf(_gameId, proposalId),
            originalWeight,
            "total attestation count should only be the original holder's weight"
        );

        // The attestation count should NOT exceed MAX_ATTESTATION_POWER_TIER for a single tier.
        assertLe(
            _gov.attestationCountOf(_gameId, proposalId),
    ...
    /// @notice Verify that a transfer in a previous block still allows the recipient to attest normally.
    function test_previousBlockTransferRecipientGetsFullWeight() external {
        _setupGame(2, 1 ether);
        _toScoring();
        ...
        // Delegate to self so recipient has attestation units.
        ...
        // Advance time by 1 second so the transfer is in a previous block.
        vm.warp(block.timestamp + 1);

        // Now the recipient should be able to attest with full weight.
        vm.prank(recipient);
        uint256 recipientWeight = _gov.attestToScorecardFrom(_gameId, proposalId);
        assertGt(recipientWeight, 0, "recipient should have attestation weight after 1 second");
    }
    ...
    function _addr(uint256 i) internal pure returns (address) {
        return address(bytes20(keccak256(abi.encode("fix-same-block", i))));
    }
